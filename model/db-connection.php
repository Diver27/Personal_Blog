...
//        echo "连接成功\n";
    ...
    public function createBlog($title, $short_desc, $idCategory, $content)
    {
        $sql = $this->conn->prepare("INSERT INTO Blog (title,short_desc,idCategory,content) VALUE (?,?,?,?)");
        $sql->bind_param("ssis", $title, $short_desc, $idCategory, $content);
        if ($sql->execute() === TRUE) {
            echo "新记录插入成功\n";
        } else {
            echo "Error: " ;
...
switch ($_POST['action']) {
    case "createBlog":
        echo $_POST['category'];
        $db->createBlog($_POST['title'], $_POST['short_desc'], $_POST['category'], $_POST['content']);
        break;
}